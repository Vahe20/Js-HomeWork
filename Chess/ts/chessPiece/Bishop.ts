...
export class Bishop extends ChessPiece {
    ...
    getAvailableMoves(chessBoard: ChessBoard) {
        ...
        for (const direction of directions) {
            ...
            while (newRow >= 0 && newRow < 8 && newCol >= 0 && newCol < 8) {
                ...
                if (field) {
                    if (field.getColor() !== this.color) moves.push({row: newRow, col: newCol});
                    break;
                }
    ...
    clone(): ChessPiece | undefined {
        return new Bishop(this.color, this.type, {row: this.position.row, col: this.position.col}, this.img);
    }
