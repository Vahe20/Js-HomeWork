...
export class Render {
    ...
    static renderCheck(chessBoard: ChessBoard, row: number, col: number) {
        const tmp = document.querySelector(`.cell-${row}-${col}`) as HTMLDivElement;
    ...
    static renderMath(chessBoard: ChessBoard) {
        ...
        const menuImg = document.getElementById('menu_img') as HTMLImageElement | null;

        if (menu) menu.style.transform = 'scale(1)';
        if (winner) winner.textContent = `${winnerColor} win!`;
        if (menuImg) menuImg.src = `../assets/images/${color === 'white' ? 'black' : 'white'}_king.png`;
    }
